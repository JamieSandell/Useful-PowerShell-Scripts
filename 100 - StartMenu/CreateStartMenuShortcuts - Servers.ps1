...
{
    $result = ""
   # ([string]$32bitPath, [string]$64bitPath, [string]$targetFile, [string]$shortcutDir, [string]$shortcutName, [string]$iconLocation, [string]$arguments)
...
$result += CreateShortcutIfRequired `
	    ...
	    "$Env:ProgramData\Microsoft\Windows\Start Menu\Programs\" `
        "Close All.lnk" `
	    "c:\sqlapps\appkill\shell32-27.ico" `
	    '/c "C:\sqlapps\AppKill\CloseAll.bat"'
    return $result
}
